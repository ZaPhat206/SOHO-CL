...
from methods.pps_soho import create_learner as create_pps_learner
from methods.cars_fly import create_learner as create_cars_learner
...
PPS_CACHE_METHODS={"pps_class_protected", "pps_standard_fd"}
CARS_CACHE_METHODS={"cars_fly"}
CACHE_REFERENCE_METHODS={
 "cached_flycl", "cached_soho_replay",
 "cached_flycl_fidelity", "cached_soho_replay_fidelity",
}
METHODS=sorted(TSOHO_METHODS | SFT_CACHE_METHODS | CRT_CACHE_METHODS | PPS_CACHE_METHODS | CARS_CACHE_METHODS | CACHE_REFERENCE_METHODS)
SCHEMA_VERSION=1


def create_cached_learner(args, method, feature_dim, ridge_lambda, rank, kappa=None, delta=None, residual_ridge=None, complement_ridge=None, confusion_temperature=None, pps_gamma=None):
 ...
 if method in PPS_CACHE_METHODS:
  dtype={"float32":torch.float32,"float64":torch.float64}[getattr(args,"pps_statistics_dtype","float64")]
  mode="class_protected" if method=="pps_class_protected" else "global"
  gamma=1.0 if mode=="global" else float(getattr(args,"pps_gamma",1.0) if pps_gamma is None else pps_gamma)
  return create_pps_learner(raw_dim=feature_dim,anchor_dim=int(getattr(args,"pps_anchor_dim",2048)),
                            synaptic_degree=int(getattr(args,"pps_synaptic_degree",300)),
                            coding_level=float(getattr(args,"pps_coding_level",.3)),sketch_size=max(int(rank),1),
                            ridge_lambda=ridge_lambda,gamma=gamma,mode=mode,seed=args.seed,device=args.device,dtype=dtype)
 if method in CARS_CACHE_METHODS:
  dtype={"float32":torch.float32,"float64":torch.float64}[getattr(args,"cars_statistics_dtype","float64")]
  return create_cars_learner(raw_dim=feature_dim,anchor_dim=int(getattr(args,"cars_anchor_dim",1024)),
                             synaptic_degree=int(getattr(args,"cars_synaptic_degree",300)),
                             coding_level=float(getattr(args,"cars_coding_level",.3)),anchor_ridge=ridge_lambda,
                             residual_ridge=float(getattr(args,"cars_residual_ridge",.1)),
                             complement_ridge=float(getattr(args,"cars_complement_ridge",.1)),
                             energy_threshold=float(getattr(args,"cars_energy_threshold",.9)),
                             max_rank=max(int(rank),1),min_rank=int(getattr(args,"cars_min_rank",1)),
                             minimum_objective_gain=float(getattr(args,"cars_minimum_objective_gain",0.)),
...
def select_config(args):
 ...
 ranks=[int(x) for x in args.search_ranks.split(",")];lambdas=[float(x) for x in args.search_lambdas.split(",")];kappas=[float(x) for x in getattr(args,"search_kappas",str(getattr(args,"fisher_kappa",1.0))).split(",")];deltas=[float(x) for x in getattr(args,"search_deltas",str(getattr(args,"fisher_delta",.1))).split(",")];residual_ridges=[float(x) for x in getattr(args,"search_crt_residual_ridges","0.01,0.1,1.0").split(",")];complement_ridges=[float(x) for x in getattr(args,"search_crt_complement_ridges","0.01,0.1,1.0").split(",")];temperatures=[float(x) for x in getattr(args,"search_crt_temperatures","0.25,0.5,1.0").split(",")];pps_gammas=[float(x) for x in getattr(args,"search_pps_gammas","0.5,1.0,2.0").split(",")];results=[]
 requested_methods=args.search_methods.split(",");pps_codes=None
 if any(method in PPS_CACHE_METHODS for method in requested_methods):
  prototype=create_cached_learner(args,"pps_class_protected",train["features"].shape[1],lambdas[0],max(ranks),pps_gamma=1.0)
  code_parts=[];code_batch_size=int(getattr(args,"pps_code_batch_size",2048));started=time.perf_counter()
  for start in range(0,len(train["features"]),code_batch_size):
   code_parts.append(prototype.encode(train["features"][start:start+code_batch_size]).detach().cpu())
   print(f"PPS WTA cache {min(start+code_batch_size,len(train['features']))}/{len(train['features'])}",flush=True)
  pps_codes=torch.cat(code_parts);del prototype,code_parts
  if torch.cuda.is_available(): torch.cuda.empty_cache()
  print(f"PPS WTA train-only cache ready shape={tuple(pps_codes.shape)} elapsed={time.perf_counter()-started:.1f}s",flush=True)
      ...
      for residual_ridge in (residual_ridges if method in CRT_CACHE_METHODS and method!="crt_anchor_only" else [None]):
       for complement_ridge in (complement_ridges if method in CRT_CACHE_METHODS and method not in {"crt_anchor_only","crt_confusion_no_residualization"} else [None]):
         for temperature in (temperatures if method in {"crt_confusion_residual","crt_shuffled_confusion_residual","crt_confusion_no_residualization"} else [None]):
          for pps_gamma in (pps_gammas if method=="pps_class_protected" else [None]):
           candidate_started=time.perf_counter();print(f"START method={method} rank={rank} lambda={ridge_lambda} gamma={pps_gamma}",flush=True)
           learner=create_cached_learner(args,method,train["features"].shape[1],ridge_lambda,rank,kappa,delta,residual_ridge,complement_ridge,temperature,pps_gamma);scores=[]
           for task in range(args.num_tasks):
            if method in PPS_CACHE_METHODS: learner.update_from_codes(pps_codes[train_parts[task]],train["labels"][train_parts[task]])
