...
Seed policy for new studies:

- use `2025` as the default seed in every newly created protocol and config;
- give every compared method in one study the exact same seed and class/task order;
- pre-register any multi-seed list before evaluating the held-out test set; the recommended new-study list is `2025, 3407, 4421, 5501, 6619`;
- never change or relabel the seed of a completed run. Existing locked protocols and artifacts that used `1993` remain historical records and must retain that value.
...
| 6 — research-scale evaluation | Seeds/datasets/ablations pre-registered. | Exact commands generated from committed configs, e.g. `python tools/run_protocol.py --manifest configs/t_soho/<study>.yaml --seeds 2025 3407 4421 5501 6619` (only if/when that tool is implemented). | Only permitted after Phase 5 report is reviewed and the low-rank/geometry hypothesis survives controls. |
