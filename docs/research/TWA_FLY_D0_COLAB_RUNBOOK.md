...
3. Run Cells 3–5. Cell 4 displays byte progress while restoring caches. D0 uses
   its own `twa_fly_d0_wta_h10000_seed1993` cache because the older ZI cache did
   not retain its projection tensor. Cell 5
...
5. Run Cell 7. On the first run it creates WTA codes and `projection.pt`, with
   live `WTA CACHE n/50000` progress; later runs restore both. It then prints one
